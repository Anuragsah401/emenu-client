...
const NotificationListAdmin = ({list, loading}) => {
    ...
    const getTime = (orderDate) => {
      ...
      return (
        <div className="absolute w-[300px] max-h-[400px] overflow-y-auto top-[4em] right-0 bg-[#20cfbbd5] font-semibold rounded-sm border-2 border-black">
          {loading ? (
            LoadingIcon
          ) : list.length !== 0 ? (
            list.map((item, i) => (
